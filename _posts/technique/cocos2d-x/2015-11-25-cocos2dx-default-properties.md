...
##前言
本文总结了cocos2d-x中与图形渲染相关的常用组件，给出其在cocos2d-x继承体系中的类图、默认属性等信息。这里说的组件主要是指跟图形渲染相关的东西，比如Node、Scene、Layer等等, 默认属性是指在调用create()方法之后的属性。同时，结合类图总结了它们之间的继承等关系。因为作者水平有限，对相关知识的理解和总结难免有错误，如您发现错误，还望给予指正，救人一错，胜造七级浮屠,非常感谢！
